...
- **Knowledge promotion check.** Every `done` project's last timeline entry must be `Checked knowledge promotion`, recorded only after the real review happened. `condash projects close` records it automatically; for a project that reached `done` another way, do the review then `condash projects check-knowledge <slug> --record` — never hand-type the marker. The gate itself is defined once, in the `/knowledge` skill's `promotion.md`; the close-time sequence is in [close.md](close.md).
