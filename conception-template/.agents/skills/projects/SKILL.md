...
Reading a file directly is always fine. **Writing** goes through the skill or a `condash` verb — the shipped `## General` region of this conception's `AGENTS.md` owns that rule, and it has no exception for a small edit. The one case it does allow is prose inside a file the CLI already created: appending to a `notes/NN-*.md` body, or the README's `## Notes` / `## Timeline` / `## Steps` lines that [update.md](update.md) tells you to edit. Creating an item, changing `status`, and every worktree action are CLI-only.
...
- `done` — finished. Folder does not move. A completed knowledge-promotion review is acknowledged only when `Checked knowledge promotion` is its last timeline entry; without that explicit acknowledgement, or after a later entry, `condash audit --include check-knowledge` reports it.

Kind-specific additions (incidents only): `environment: <PROD/STAGING/DEV>`, `severity: <low/medium/high — impact>`.

`apps` is a YAML list (one entry per line). `branch`'s value is authoritative. `date` always matches the month directory — changing it requires a `git mv`, which is the user's step (agents run no `git` in the conception checkout).
...
- **Do not commit or push.** The `## General` region of this conception's `AGENTS.md` owns this rule — read it there. What is specific to closing an item: a sweep that introduces the `Closed.` timeline entry mints the `Close <slug>. Outcome: …` subject itself, so there is nothing to write by hand.
...
- **Knowledge promotion check.** An ordinary `condash projects close` does not record `Checked knowledge promotion`. After the real review, either close with `--knowledge-checked` or, for an already-done project, run `condash projects check-knowledge <slug> --record` — never hand-type the marker. Both write it last; a later entry invalidates the acknowledgement. The gate itself is defined once, in the `/knowledge` skill's `promotion.md`; the close-time sequence is in [close.md](close.md).
