...
| `check-knowledge` | `/projects check-knowledge <slug>` — append knowledge check timeline entry | [close.md](close.md)       |
...
- **Knowledge promotion check.** Every `status: done` project's last timeline entry must be `Checked knowledge promotion`. `condash projects close` appends it automatically. If a project reaches `done` without it (manual status change, old project), run `condash projects check-knowledge <slug>` to append it. The `condash audit --include knowledge-check` flag reports all done projects missing the check.
