...
- **Do not commit or push.** `condash sync` is the conception's only committer — write the files and stop. `condash sync run` sweeps them; `condash sync commit <item> --message "<subject>"` is the milestone commit. Never `git add` / `git commit` / `git push` in the conception checkout (a repo worktree is a different tree — commit there as normal).
