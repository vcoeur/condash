...
| `question-form` | `<QuestionForm>` | questions, submitLabel? | An open-questions form (single/multi/freeform, recommended options). condash renders it interactively: the reader answers in the viewer, and one Save (head button or Ctrl+S) writes the answers from every form in the document back into the same .mdx. submitLabel is still round-tripped but no longer labels a per-form button. |
