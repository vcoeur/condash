...
- **Agents** replace the old tab-strip launchers (removed). An *agent* = a harness + a harness-specific config + an optional API token, named `<harness>-<model_variant>` (derived, never stored). Definitions live as one JSON file each at `<conception>/agents/<name>.json` (loaded by `src/main/agents.ts`, validated with zod, **not** a `condash.json` key); tokens live in gitignored `<conception>/agents/.env` (`NAME=value`) and are read **only in the main process** — the renderer sees token *presence* and `$SECRET_ENV` references, never values. The Agents pane (`src/renderer/panes/agents.tsx`, working surface `'agents'`) does CRUD + launch + a read-only "view config"; the tab-strip spawn dropdown lists agents (`TermSpawnRequest.agentName` → `resolveAgentSpawn` → env merged into the child in `terminals.ts`). IPC: `listAgents` (each item carries a `command` preview line) / `readAgent` / `writeAgent` / `deleteAgent` / `previewAgent` / `openAgentsEnv` (creates `agents/.env` from a template if missing, then opens it). A claude agent with an empty `baseUrl` is "native" — `buildSpawn` runs bare `claude` (no env). opencode's `--model` is its *default* model; per-agent (plan/build) models live in the project's `opencode.json`, not a launch flag. Migration: `migrateRawSettings()` drops legacy `terminal.launchers` / `launcher_command` and renames action-template `launcher` → `agent`.
