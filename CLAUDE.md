...
- **Tasks** are reusable, parameterized agent prompts. A task is a referenced agent + a markdown prompt with fillable `{markers}`, stored as one directory each at `<conception>/tasks/<slug>/`: `task.json` (`name` / `agent` / `submit`) + a hand-editable `prompt.md` (prose stays in markdown, config in JSON — the conception "never mix shapes" rule). Loaded by `src/main/tasks.ts` (mirrors `agents.ts`: ENOENT-tolerant `listTasks`, slug-validated writes via `isValidSlugTail`, idempotent `deleteTask`); IPC `listTasks` / `readTask` / `writeTask` / `deleteTask` (`src/main/ipc/tasks.ts`). The Tasks pane (`src/renderer/panes/tasks.tsx`) is a **left-band view** (`LeftView` adds `'tasks'`, ordered Projects · Tasks · Deliverables) — card list + a fill view (pickers + prefilled fields + live preview → Run) + an editor. Marker grammar + the `{APP_*}` / `{PROJECT_*}` reserved-token context builders live in `src/shared/tasks.ts` (`extractMarkers`, `appContext`, `projectTokenContext`); `substitute` (`src/shared/action-template.ts`) now honours `{KEY:default}` as a fallback (backward-compatible — existing `{token}`s still resolve). Run reuses the agent-spawn-and-type path: `terminal-bridge.ts`'s `runTask(agentName, text, submit)` spawns a fresh tab running the agent (cwd = conception root) and types the substituted prompt. A dangling `agent` (renamed/removed) is flagged on the card (`agentPresent`) and disables Run.
- **Agents** replace the old tab-strip launchers (removed). An *agent* = a harness + a harness-specific config + an optional API token, named `<harness>-<model_variant>` (derived, never stored). Definitions live as one JSON file each at `<conception>/agents/<name>.json` (loaded by `src/main/agents.ts`, validated with zod, **not** a `condash.json` key); tokens live in gitignored `<conception>/agents/.env` (`NAME=value`) and are read **only in the main process** — the renderer sees token *presence* and `$SECRET_ENV` references, never values. The Agents pane (`src/renderer/panes/agents.tsx`, working surface `'agents'`) does CRUD + launch + a read-only "view config"; the tab-strip spawn dropdown lists agents (`TermSpawnRequest.agentName` → `resolveAgentSpawn` → env merged into the child in `terminals.ts`). IPC: `listAgents` (each item carries a `command` preview line) / `readAgent` / `writeAgent` / `deleteAgent` / `previewAgent` / `readAgentsEnv` + `writeAgentsEnv` (the in-app token editor — the one place secret *values* reach the renderer, by explicit user action). Per-harness `buildSpawn` specifics: a claude agent with an empty `baseUrl` is "native" (runs bare `claude`, no env); opencode inlines its config via `OPENCODE_CONFIG_CONTENT` (top-level `model` default + `agent.build.model` / `agent.plan.model` overrides + an `extraConfigJson` merge — same trick as the `opencode-deepseek-auto` wrapper, no `opencode.json` needed); kimi-cli adds `--model` / `--thinking` / `--plan` / inline `--config` alongside `--agent-file`. Agent args are POSIX-quoted in `terminals.ts` (`quoteArg`) so inline-config JSON survives shell wrapping. Migration: `migrateRawSettings()` drops legacy `terminal.launchers` / `launcher_command` and renames action-template `launcher` → `agent`.
