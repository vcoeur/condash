...
- **Per-conception logs**: `<conception>/.condash/logs/YYYY/MM/DD/HHMMSS-<sid>.jsonl`. One file per pty spawn; gitignored alongside `settings.json`. Janitor (`src/main/terminal-logger-janitor.ts`) evicts day-directories past `terminal.logging.retentionDays` (default 14) and oldest-first while over `terminal.logging.maxDirMb` (default 500). Single sessions rotate to `.2.jsonl`, `.3.jsonl`, … once they cross `terminal.logging.maxFileMb` (default 5 — small because TUIs emit constant cursor-positioned repaints that the render-side replay handles cheaply but that bloat raw bytes). Runs at startup + every 24 h. The Logs working surface (`Cmd+Shift+L`) browses the tree; `out` events are replayed through an off-screen xterm + SerializeAddon (see `src/renderer/panes/logs-replay.ts`) so cursor-motion-heavy TUI streams render as one transcript instead of one row per spinner frame.
