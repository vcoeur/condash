...
- **Terminal launchers**: `terminal.launchers: [{ symbol: 'lambda' | 'mu', command, title? }]` — one button per entry with `command` set is rendered on the tab strip (λ for `lambda`, μ for `mu`). `title`, when present, becomes the pinned tab label at spawn time; the user's inline rename still wins forever after. Legacy scalar `terminal.launcher_command` is transparently migrated into `launchers[0]` (symbol: `lambda`) by `migrateRawSettings()` in `src/main/config-schema.ts`; the legacy key is dropped on next write.
