...
The Code pane's **branch filter** (top-of-pane popover) hides every non-primary worktree row by default; ticking a branch pins it visible on every card that carries it. The primary worktree row (the checkout under `workspace_path`) is always rendered with a subtly tinted background. Hydration + persistence go through `createBranchFilterStore` in `src/renderer/branch-filter-store.ts` and the `getSelectedBranches` / `setSelectedBranches` IPC verbs (settings.json field: `selectedBranches`). Filter logic is the pure `filterWorktrees` in `src/renderer/panes/code-parts/data.ts`.
