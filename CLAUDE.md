...
- **Merge rule**: top-level replace for every key, with **one exception**: `terminal` merges one level deep so a conception customising `terminal.logging` keeps the per-machine `terminal.launchers` / `screenshot_dir` / shortcuts. The exception is documented in `docs/reference/config.md` and tested in `effective-config.test.ts`.
- **Terminal launchers**: `terminal.launchers: [{ label, command, title? }]` — each entry with `command` set renders as an option in the tab-strip dropdown. `label` is the user-defined name shown in the dropdown; `title`, when present, becomes the pinned tab label at spawn time (falls back to `command`); the user's inline rename still wins forever after. Legacy `symbol`-based entries (`lambda` → `λ`, `mu` → `μ`) are transparently migrated to `label` by `migrateRawSettings()` in `src/main/config-schema.ts`. The legacy scalar `terminal.launcher_command` is also migrated into `launchers[0]` (label: `λ`). The dropdown menu itself is rendered through `solid-js/web`'s `<Portal>` to `document.body` and positioned with `position: fixed` coords from `createDropdownMenu({ align: 'left' })` (`src/renderer/dropdown-menu.ts`) — without the portal hop, `.terminal-pane`'s `contain: layout paint` re-anchors `position: fixed` to the pane and the strip's `overflow: auto` clips the menu height down to 32 px (fixed in v3.4.1 — see `tests/spawn-dropdown-clipped.spec.ts`).
