...
const TAB_SYSTEM_PROMPT = [
  ...
  'Judge the tab state from the LATEST output, not the most dramatic line: if new',
  'output is still appearing, the tab is working, not blocked. When the latest',
  'output disagrees with the prior summary, trust the latest output.',
  'Only call a tab blocked, stalled, or waiting when the most recent output is an',
  'unanswered interactive prompt with no activity after it. Treat informational',
  'notices and recoverable warnings as background noise (for example "workspace',
  'not trusted", "N permissions ignored", deprecation or auth notices): never',
  'report them as the current action or a blocker.',
  'Prefer literal description over alarming interpretation.',
...
const OVERVIEW_SYSTEM_PROMPT = [
  ...
  'cross-tab crisis: only call the developer blocked if a tab summary clearly',
  'states it is waiting on user input right now. Treat warnings and recoverable',
  'errors as routine.',
  'Reply with ONLY a JSON object, no markdown fence:',
