...
/** Read ~/.kimi/global-agent.yaml-style file and return the ROLE_ADDITIONAL string (or empty). */
async function readKimiRoleAdditional(path: string): Promise<string> {
  const raw = await fs.readFile(path, 'utf8');
  const doc = parseYaml(raw) as {
    agent?: { system_prompt_args?: { ROLE_ADDITIONAL?: string } };
  };
  return doc?.agent?.system_prompt_args?.ROLE_ADDITIONAL ?? '';
}
...
describe('condash skills install --user (agent-config compile)', () => {
  ...
  it('compiles common.md with no per-agent fragments to both outputs', async () => {
    ...
    // Kimi: yaml file with the compiled content under ROLE_ADDITIONAL.
    const kimiYaml = await fs.readFile(kimiAgentOutput, 'utf8');
    const { parse: parseYaml } = await import('yaml');
    const kimiDoc = parseYaml(kimiYaml) as {
      version?: number;
      agent?: { extend?: string; system_prompt_args?: { ROLE_ADDITIONAL?: string } };
    };
    expect(kimiDoc.version).toBe(1);
    expect(kimiDoc.agent?.extend).toBe('default');
    const kimiContent = kimiDoc.agent?.system_prompt_args?.ROLE_ADDITIONAL ?? '';
  ...
  it('splices per-agent fragments before ## Specifics', async () => {
    ...
    await install();
    const claudeContent = await fs.readFile(claudeAgentOutput, 'utf8');
    const kimiContent = await readKimiRoleAdditional(kimiAgentOutput);
  ...
  it('regenerates outputs on every install (idempotent)', async () => {
    ...
    const v1Kimi = await readKimiRoleAdditional(kimiAgentOutput);
    await install();
    expect(await fs.readFile(claudeAgentOutput, 'utf8')).toBe(v1Claude);
    expect(await readKimiRoleAdditional(kimiAgentOutput)).toBe(v1Kimi);
  ...
  it('Kimi: embeds compiled content under agent.system_prompt_args.ROLE_ADDITIONAL', async () => {
    await writeAgentConfig({ common: '# Hi\n' });
    await install();
    const raw = await fs.readFile(kimiAgentOutput, 'utf8');
    expect(raw).toContain('version: 1');
    expect(raw).toContain('extend: default');
    expect(raw).toContain('ROLE_ADDITIONAL: |');
    // ROLE_ADDITIONAL must contain the generated-by header + body content.
    const role = await readKimiRoleAdditional(kimiAgentOutput);
    expect(role).toMatch(/^<!-- Generated by `condash skills install` /);
    expect(role).toContain('# Hi');
  });

  it('Kimi: preserves user-added fields in global-agent.yaml across install', async () => {
    // Pre-seed the yaml with a custom field that condash should NOT remove.
    await fs.mkdir(join(kimiAgentOutput, '..'), { recursive: true });
    const preexisting = stringifyYaml(
      {
        version: 1,
        agent: {
          extend: 'default',
          system_prompt_args: { ROLE_ADDITIONAL: 'old content' },
          custom_user_field: { keep: 'me' },
        },
        another_top_level: ['x', 'y'],
      },
      { blockQuote: 'literal', lineWidth: 0 },
    );
    await fs.writeFile(kimiAgentOutput, preexisting);
    await writeAgentConfig({ common: '# New rules\n' });
    await install();
    ...
    const doc = parseYaml(raw) as {
      ...
      agent: {
        extend: string;
        system_prompt_args: { ROLE_ADDITIONAL: string };
        custom_user_field?: { keep?: string };
      };
      another_top_level?: string[];
    };
    expect(doc.agent.system_prompt_args.ROLE_ADDITIONAL).toContain('# New rules');
    expect(doc.agent.custom_user_field?.keep).toBe('me');
    expect(doc.another_top_level).toEqual(['x', 'y']);
  });

  it('Kimi: creates global-agent.yaml from scratch when absent', async () => {
    await writeAgentConfig({ common: '# Fresh install\n' });
    await install();
    const doc = parseYaml(await fs.readFile(kimiAgentOutput, 'utf8')) as {
      version: number;
      agent: { extend: string; system_prompt_args: { ROLE_ADDITIONAL: string } };
    };
    expect(doc.version).toBe(1);
    expect(doc.agent.extend).toBe('default');
    expect(doc.agent.system_prompt_args.ROLE_ADDITIONAL).toContain('# Fresh install');
  });
