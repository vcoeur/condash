...
To control reasoning effort (and verbosity / summary), define **named variants** in condash's opencode agent editor — each variant bundles `reasoningEffort` (`none`/`minimal`/`low`/`medium`/`high`/`xhigh`), `textVerbosity` (`low`/`medium`/`high`), and `reasoningSummary` (`auto`/…). Pick a **default variant** (applied to every agent) and optionally assign per-agent variants for exceptions (e.g. `plan` → a deeper variant). condash inlines these as `provider.<id>.models.<model>.variants` plus `agent.<name>.variant` in `OPENCODE_CONFIG_CONTENT`. In the TUI, **ctrl+t** (OpenCode's built-in `variant_cycle`) switches variants live and the status footer shows the active one. (Equivalently, you can hand-write `variants` under a model in `opencode.json`.) Providers that support a reasoning budget (e.g. DeepSeek) map `reasoningEffort` onto their own setting; leave variants unset to use the model default.
...
3. **Config** — depends on the harness:
   ...
   - **opencode** — set the **default model** (`provider/model`) plus optional **build** / **plan** agent-model overrides; define **named variants** (each: name + `reasoningEffort` / `textVerbosity` / `reasoningSummary`), pick a **default variant** applied to every agent, and add **per-agent variant** exceptions; plus an "Extra config (JSON)" escape hatch. condash inlines all of it as `OPENCODE_CONFIG_CONTENT` (build/plan become `agent.build.model` / `agent.plan.model`; variants become `provider.<id>.models.<model>.variants`; the default + per-agent variants become `agent.<name>.variant`) — no `opencode.json` needed. **ctrl+t** cycles variants live in the TUI.
