...
To control reasoning per agent, condash's opencode agent editor shows a single **options table**: a **default** row (the default model + `reasoningEffort` (`none`/`minimal`/`low`/`medium`/`high`/`xhigh`), `textVerbosity` (`low`/`medium`/`high`), `reasoningSummary` (`auto`/…)) applied to every agent, plus **per-agent rows** that override the **model** and/or those options (e.g. `plan` on `kimi-for-coding/kimi-k2-thinking` at `high`, `build` on the default `deepseek` model at `low`). Under the hood condash turns the table into opencode **variants named by effort** — `provider.<id>.models.<model>.variants` for every referenced model plus `agent.<name>.variant` (= the agent's effort) and `agent.<name>.model` — so the TUI status **footer shows each agent's effort** and **ctrl+t** (OpenCode's built-in `variant_cycle`) cycles them. (Equivalently, you can hand-write `variants` under a model in `opencode.json`.) Providers that support a reasoning budget (e.g. DeepSeek) map `reasoningEffort` onto their own setting; leave effort blank to use the model default. Note: a model belongs to the **agent** (`agent.<name>.model`), not to a variant.
