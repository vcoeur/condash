...
To control reasoning per agent, condash's opencode agent editor shows a single **options table**: a **default** row (the default model + `reasoningEffort` (`none`/`minimal`/`low`/`medium`/`high`/`xhigh`), `textVerbosity` (`low`/`medium`/`high`), `reasoningSummary` (`auto`/…)) applied to every agent, plus **per-agent rows** that override the **model** and/or those options (e.g. `plan` on `kimi-for-coding/kimi-k2-thinking` at `high`, `build` on the default `deepseek` model at `low`). Under the hood condash turns the table into opencode **variants named by effort** — `provider.<id>.models.<model>.variants` for every referenced model plus `agent.<name>.variant` (= the agent's effort) and `agent.<name>.model`. Each agent's effort is **applied to its requests** (verified on the wire); **ctrl+t** (OpenCode's built-in `variant_cycle`) cycles the efforts live in the TUI. (opencode's status footer reflects that live ctrl+t selection — it does **not** show the per-agent configured effort on launch.) (Equivalently, you can hand-write `variants` under a model in `opencode.json`.) Providers that support a reasoning budget (e.g. DeepSeek) map `reasoningEffort` onto their own setting; leave effort blank to use the model default. Note: a model belongs to the **agent** (`agent.<name>.model`), not to a variant.
...
3. **Config** — depends on the harness:
   ...
   - **opencode** — fill the **per-agent options table**: the `default` row sets the default model + reasoning effort / verbosity / summary for every agent, and each added row overrides one agent's model and/or options (e.g. plan-on-one-model, build-on-another); plus an "Extra config (JSON)" escape hatch. condash translates the table into `OPENCODE_CONFIG_CONTENT` — variants named by effort under `provider.<id>.models.<model>.variants`, with `agent.<name>.variant` + `agent.<name>.model` — so each agent's effort reaches its requests and **ctrl+t** cycles them live (the footer reflects the live ctrl+t selection, not the per-agent default). No `opencode.json` needed.
