...
To control reasoning effort (and verbosity / summary), define **named variants** in condash's opencode agent editor — each variant bundles `reasoningEffort` (`none`/`minimal`/`low`/`medium`/`high`/`xhigh`), `textVerbosity` (`low`/`medium`/`high`), and `reasoningSummary` (`auto`/…). Pick a **default variant** (applied to every agent) and add **per-agent overrides** for exceptions — each gives one agent its own **model** and/or **variant** (e.g. `plan` on `kimi-for-coding/kimi-k2-thinking`, `build` on the default `deepseek` model). condash inlines these as `provider.<id>.models.<model>.variants` (for every referenced model) plus `agent.<name>.model` / `agent.<name>.variant` in `OPENCODE_CONFIG_CONTENT`. In the TUI, **ctrl+t** (OpenCode's built-in `variant_cycle`) switches variants live and the status footer shows the active one. (Equivalently, you can hand-write `variants` under a model in `opencode.json`.) Providers that support a reasoning budget (e.g. DeepSeek) map `reasoningEffort` onto their own setting; leave variants unset to use the model default. Note: a model belongs to the agent (`agent.<name>.model`), **not** to a variant — a variant carries only generation options.
...
3. **Config** — depends on the harness:
   ...
   - **opencode** — set the **default model** (`provider/model`); define **named variants** (each: name + `reasoningEffort` / `textVerbosity` / `reasoningSummary`), pick a **default variant** applied to every agent, and add **per-agent overrides** (each: agent + optional own **model** + optional **variant**) for exceptions like plan-on-one-model, build-on-another; plus an "Extra config (JSON)" escape hatch. condash inlines all of it as `OPENCODE_CONFIG_CONTENT` (variants become `provider.<id>.models.<model>.variants`; the default + per-agent variants become `agent.<name>.variant`; a per-agent model becomes `agent.<name>.model`) — no `opencode.json` needed. **ctrl+t** cycles variants live in the TUI.
