...
The button can also **suggest** the link. condash scans each tab's recent output for project slugs, short slugs and branch names, and when the focused tab keeps naming one project the card's button reads **Link (suggested)** in the link accent — so a tab that is plainly working on a project offers the link instead of waiting to be told. Only non-`done` projects are recognised, a window that names several projects at once (a listing, a tree-wide grep) is treated as evidence for nothing, and an ambiguous window suggests nothing rather than guessing. A suggestion is **not** a link: it decorates only the button, it drives neither the card highlight nor the Active-tab filter, and clicking writes exactly the ordinary relation the plain button writes. Nothing is scanned or stored beyond the app, and the matched text is never shown.
