...
Cards and status sections have no nested outline. An ordinary card separates from the status wash through its elevated background and spacing, while each populated status section remains identifiable through its tinted wash, header, and dot; the outer Projects pane stays unframed. Colour on a card means only "belongs together": a parent and its subprojects share one hue on their directional edges and a hint of it in the title, so a plan and its spin-offs read as one group at a glance. A card with children wears a narrow **solid left** edge, a card that declares a `parent:` wears a narrow **dashed right** edge, and a mid-tree node wears both at once — hierarchy is decoration only, never reordering, indenting, or nesting cards. A card whose only relation is a `parent:` that no longer resolves keeps the dashed right edge in the neutral colour — there is no second card for a hue to tie it to, but the declared edge still shows. A card in a family also grows a **Part of ↑** banner (child) or a **Subprojects** fold (parent) — collapsed by default, one row per child once opened; the open/closed state is remembered per parent. See [Parent / subprojects](../reference/readme-format.md#parent-subprojects).

### Linking a card to terminal tabs { #card-tab-links }

Cards can be **linked** to the embedded terminal's tabs (see [Use the embedded terminal](terminal.md)): click the **Link** button in the card's chrome to bind the card to the currently focused tab. Each click adds one relation — many-to-many, never a replace, so one card can link any number of tabs and one tab any number of cards. The button is **disabled until a tab is focused** — linking never spawns a tab you didn't ask for; open one first — and it **disappears once the focused tab is already linked** to this card: a second link of the same pair would change nothing, and undoing the link lives on the Linked-tabs fold, which is exactly where the eye already is in that state.

A link can also be written **for you, by a project action**. The built-in **Work on `<slug>`** row always links the tab it types into — starting work on a project is exactly the moment its tab belongs to it — and any entry in [`terminal.projectActions`](../reference/config.md#terminalprojectactions) does the same when its **Link tab** flag is set. Unlike the Link button, these can link a tab they spawned: Work-on opens a shell when the pane is empty, and an agent-bound action links the tab the agent runs in, not the one that held focus when you clicked. Linking stays additive, so firing the same action twice changes nothing.

The button can also **suggest** the link. condash scans each tab's recent output for project slugs, short slugs and branch names, and when the focused tab keeps naming one project the card's button reads **Link (suggested)**, filled with the accent colour — the strongest treatment on the card frame short of a real relation — so a tab that is plainly working on a project offers the link instead of waiting to be told. Only non-`done` projects are recognised, a window that names several projects at once (a listing, a tree-wide grep) is treated as evidence for nothing, and an ambiguous window suggests nothing rather than guessing. A suggestion is **not** a link: it decorates only the button, it drives neither the card highlight nor the Active-tab filter, and clicking writes exactly the ordinary relation the plain button writes. The scan keeps up with brand-new projects too — a tab that printed a project's slug before that project existed (the `condash projects create` case, where the tab goes quiet the moment the command returns) is re-scored the moment the needle set learns the slug. Nothing is scanned or stored beyond the app, and the matched text is never shown.

A linked card shows a **"Linked tabs" fold** in its relations zone — a caret + "Linked tabs" + the tab count, **collapsed by default**, its open/closed state remembered per card like the Subprojects fold. Expanding it lists the linked tabs (one row per tab, the label captured at link time), each with a **focus** arrow — activates that tab in the terminal — and an **×** that unlinks exactly that one. The header is tinted with the link accent and carries a **full-width accent underline** — **subtle** (a washed-out underline) while any linked tab is live, **stronger** (a solid underline, accent-filled count) while the currently focused tab is among them. While the focused tab is linked, the whole card also wears a **prominent highlight** — an accent-tinted background and an accent ring around the card — which stays distinct from the family-coloured hierarchy edges, hover, the keyboard focus ring, and the star, and disappears the moment the focused tab is not among the linked ones (or the last tab unlinks, from the fold or the tab's context menu, or because the tab closed).

Links are **session-lifetime**: they are stored locally (a versioned `localStorage` key, never the tree or `settings.json`), they survive an app reload while their tabs are alive, and they **die with their tab** — closing a tab removes every relation of it, so the fold rows and the decoration clear reactively (re-linking is one click). Restarting a tab re-points its links onto the new session. A card row's label is captured when the link is made and stays until the pair is unlinked and re-linked; the tab itself always shows its true name. On the tab side, the linked projects appear in the tab's hover popover and can be unlinked from its context menu — the tab itself is never decorated. See [Use the embedded terminal → Tab links](terminal.md#tab-links).
...
- **Active tab** — a toggle: only items [linked to the currently focused terminal tab](#card-tab-links). Disabled while no tab is focused (the **Link** button never spawns a tab, so with none open there is nothing to filter by); with the toggle on and an unlinked tab focused, everything hides — **0 of N** is a true result, not a bug, and the usual **Clear filters** affordance resets it.
