...
A linked card shows a **"Linked tabs" fold** in its relations zone — a caret + "Linked tabs" + the tab count, **collapsed by default**, its open/closed state remembered per card like the Subprojects fold. Expanding it lists the linked tabs (one row per tab, the label captured at link time), each with a **focus** arrow — activates that tab in the terminal — and an **×** that unlinks exactly that one. The header is tinted with the link accent and carries a **full-width accent underline** — **subtle** (a washed-out underline) while any linked tab is live, **stronger** (a solid underline, accent-filled count) while the currently focused tab is among them. There is no accent on the card's left edge: the left edge carries only the family frame. When the last tab unlinks — in the fold or from the tab's context menu, or because the tab closed — the fold disappears entirely.
