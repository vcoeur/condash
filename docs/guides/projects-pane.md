...
A linked card shows a **"n tabs" chip** in the meta row — a button: click it to open a small popover listing the linked tabs (one row per tab, the label captured at link time), each with a **focus** arrow — activates that tab in the terminal — and an **×** that unlinks exactly that one. The popover closes on outside-click, Escape, or when the last relation goes away. The chip and the accent strip on the card's flat left edge are **subtle** (a thin edge + bordered chip) while any linked tab is live, **stronger** (the edge grows to the card's full height, the chip fills) while the currently focused tab is among them. The family left-edge hue is untouched in either state — the link accent is the neutral UI accent.
