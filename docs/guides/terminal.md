...
3. Inserts its absolute path at the active tab's prompt — no `Enter` appended; you confirm. The tab is addressed by session, not by "whatever has focus", so a tab opening elsewhere at that moment cannot catch the paste. When the active column has no live tab to paste into — it is empty, or the tab there has exited — nothing is pasted and it says so rather than failing silently. It looks only at the active column, so a live shell in the *other* column of a split does not count. Unlike the Resources pane's **→ term**, this shortcut never opens a shell for you: it is key-repeatable, and holding it would open a tab per repeat. It does bring the terminal body up when the bottom band is showing the Dashboard, so you can see the path it just pasted — the only layout change it makes, and only once the paste has landed.
