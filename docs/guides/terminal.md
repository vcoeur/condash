...
- **Dashboard tab** — the left strip's first entry is a fixed **Dashboard** tab (it can't be renamed, closed, or dragged). Selecting it swaps the bottom band to the live tab-summaries Dashboard — an always-on status strip (the next-update countdown, what the summarizer loop is doing right now, and the last-run time, so an idle-but-running engine is never mistaken for a dead one), the active summarizer settings, a card per **open tab** (a rich summary when one exists, otherwise a fallback card from the tab's command and cwd that shows when its first output is expected, so no open tab is ever hidden; a card whose summary is being recomputed that cycle keeps its own state badge and colour and shows a small pulsing dot — the loud **Summarizing** label and its accent live only in the status strip), the cross-tab **Global current work** summary (a one-line Level-1 headline of the overarching work across the tabs, then the Level-2 detail / current-activity lines — which narrate open-but-unsummarized tabs until real summaries arrive), and any last-cycle error. Selecting any real terminal tab switches straight back to that terminal. It's always present, even with the summary feature off (the body then explains how to enable it).
