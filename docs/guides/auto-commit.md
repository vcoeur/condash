...
4. **Then the regenerated indexes**, in a commit of their own (`indexes: sync`). An index is fan-in over every item of its tree, so regenerating one while an item of that tree is mid-write would record a bullet pointing at an uncommitted directory. A held-back path therefore defers index work — but only for **its own tree**: a mid-write project item never delays a `knowledge/**/index.md`, and vice versa. The sweep names what it deferred (`deferred projects indexes until that tree settles`), so an index change that sits uncommitted across several sweeps is visible rather than silent.
