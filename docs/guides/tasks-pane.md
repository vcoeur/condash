...
|--------|-------|-----------|------------|
| `{APP}` | app picker | `#<handle>` (e.g. `#kasten` for `notes.vcoeur.com`) | `{APP_NAME}` (repo name), `{APP_PATH}` (absolute checkout path) |
...
1. Open the **Automations** rail item and click **Run…** on a task card.
