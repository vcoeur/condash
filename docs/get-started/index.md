...
- **Code** (right pane) — your repos, dirty status, open-in-IDE buttons. Empty until you set `workspace_path` and `repositories` in `condash.json` (Settings → **Workspace** + **Repositories**). The legacy filename `configuration.json` is still read as a fallback.
