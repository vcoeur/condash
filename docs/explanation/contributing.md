...
Package installs go through the local composite action **`.github/actions/apt-install`** rather than a bare `apt-get` — it bounds each call with `timeout -k`, retries the update and install legs separately, repairs an interrupted dpkg between attempts, and can skip entirely when the binary is already on the runner image. Two traps are worth knowing before editing any of this:

- A step-level `timeout-minutes` on a step that `uses:` a composite action **does not** propagate into that action's steps ([actions/runner#1979](https://github.com/actions/runner/issues/1979)). The action carries its own `timeout`, and the calling job's ceiling is the outer net; the step-level ones are backstops for the day that changes.
- `npx playwright install --with-deps` runs apt *inside* itself, out of reach of the action. `_playwright.yml` therefore writes `/etc/apt/apt.conf.d/99-ci-timeouts` in a step of its own beforehand — apt ships with `Acquire::Retries` at 0, so without it a dropped index fetch is simply lost.
