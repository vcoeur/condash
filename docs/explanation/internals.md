...
`git-status-cache.ts` caches per-working-tree dirty counts for `STATUS_TTL_MS` — **60 s** (3 s originally, 12 s from #475, 60 s from the B1a idle-poll fix: the status bar polls the conception's dirty/upstream snapshot every 20 s, so a 12 s window sat below the poll cadence and every tick was a guaranteed cache miss — ~8,200 git spawns/day idle). The Refresh button calls `invalidateAll()` before re-reading, so an explicit user request always sees fresh data. Ambient re-renders (pane switch, tree-events) hit the cache.
...
- `ignore` — a `projects/**/index.md` regen (or any file above the slug level): store-irrelevant, touches nothing. The search index is still kept fresh independently upstream. Also any event under `resources/local/` — that directory is gitignored agent/task scratch (conception convention), and its constant write churn was the trigger of a resources-tree reload storm: it is excluded from the chokidar watch set outright (`watcher.ts`), and a stray event that still arrives classifies as `ignore` here. The search index already skips `local/` segments (`search/walk.ts`). Accepted trade: an *open* Resources pane no longer live-updates for changes inside `local/` (the tree walk still includes it) — the view self-heals on the next non-`local/` resources event, pane switch, or F5.
...
- **Set membership** — worktree add/remove and primary checkout branch switch flow as `repo-worktrees-changed { repoPath }`. A second per-primary watcher on `.git/HEAD` + `.git/worktrees/` fires this; debounce 250 ms; renderer responds with `listReposForPrimary` (per-primary partial reload) merged via `reconcile({ key: 'path' })`. Open popovers still survive thanks to the `path`-keyed reconcile contract. The handler re-syncs the FS-watch set **incrementally for that primary only**: every full sync attributes each watch target to its top-level primary, and a structural event replaces just that primary's slice (its path, worktrees, submodule children) with the fresh entries' targets — one `.git/HEAD` flip used to drag a whole-registry git fan-out behind it. `git worktree remove` of the *last* worktree unlinks `.git/worktrees/` itself, killing the inotify watch beneath chokidar; the structural handler detects `unlinkDir` on that path and immediately rebuilds the watcher (close + `mkdirSync` + fresh `chokidar.watch`) so the next add isn't silently missed.

F5 / View → Refresh fans out across **every working surface** (`reloadAll` in `src/renderer/hooks/use-conception.ts`): it drops the git-status TTL cache, then reloads projects, knowledge, resources, all four skill tabs, agents (the spawn-dropdown list), tasks, and logs, and calls the full `reloadRepos()` so any out-of-app worktree mutation is visible immediately. The agents list and the tasks pane own their own `createResource` and logs pushes a refresh through a deferred signal, so those three are kicked synchronously rather than awaited alongside the store reloads. Tree-store reloads — here and watcher-driven — are gated on the pane's activation latch (`createTreeStore`): a pane that was never opened holds no data, so reloading it is pure churn (a resources reload is a full recursive walk with per-markdown head reads); its first open fetches through the conception-path effect instead. Once a pane has activated, the latch stays true and every reload runs.
