...
Refresh runs automatically on switch by default. A `createEffect` diffs each column's active id against its previous value and, on a genuine switch to a different tab, calls `refreshSession(id, { onlyIfAltBuffer: !refreshAll })`, where `refreshAll` is true unless `terminal.autoRefreshOnTabSwitch` is explicitly `false`. `refreshSession` checks the tab's buffer type *after* it hydrates; when `refreshAll` is false it nudges only tabs on the alternate screen buffer (live full-screen TUIs), whose hydrated frame is inherently lossy. It captures every switch path — tab click, keyboard move, focus-promote, cross-column drag, and the active tab being closed — because they all funnel through the same active-id signal. `terminal.autoRefreshOnTabSwitch` (Settings → Terminal → "Auto-refresh on tab switch") lets users opt back into the alt-buffer-only behaviour. The repaint is deferred to a microtask so the effect never writes the active-id signal re-entrantly.
