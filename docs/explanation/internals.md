...
A *manual* Refresh that arrives while a nudge is already holding the same terminal is queued behind it, never dropped — the whole bug class is "I had to press Refresh", so swallowing a press is the one thing this path must not do. An *automatic* one is skipped, since it adds nothing to a repaint already in flight. `terminal.autoRefreshOnTabSwitch` (Settings → Terminal → "Auto-refresh on tab switch") lets users opt back into the alt-buffer-only behaviour, and applies to every automatic repaint, not just the switch path. The repaint is deferred to a microtask so the effect never writes the active-id signal re-entrantly.
