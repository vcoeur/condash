...
`SerializeAddon` round-trips a normal shell buffer faithfully, but it cannot perfectly reproduce a **live full-screen TUI** (Claude Code / any Ink or ncurses app) that repaints via relative cursor moves, scroll regions, and a bottom status line. A tab hidden mid-repaint can hydrate from a half-frame snapshot on switch-back, showing stale/duplicated rows above the live frame. The escape hatch is the **Refresh** action (`refreshSession` in `terminal-pane/controller.ts`, exposed as the tab-strip **Refresh** button and a tab context-menu item): it nudges the pty one row shorter and back (`term.resize` → `termResize` → two SIGWINCHes), which makes the running program redraw its whole screen. A brief hold between the two size deltas (`REPAINT_NUDGE_MS`) ensures the program treats it as a real resize rather than a coalesced no-op. Scrollback is kept, and plain shells ignore the resize.

Refresh is a manual action by default. Setting `terminal.autoRefreshOnTabSwitch` (Settings → Terminal → "Auto-refresh on tab switch") makes the controller run `refreshSession` automatically whenever a tab becomes its column's active tab: a `createEffect` diffs each column's active id against its previous value and fires only on a genuine switch to a different tab (skipping first-open and the no-op re-assert `refreshSession` itself makes). It captures every switch path — tab click, keyboard move, focus-promote, cross-column drag, and the active tab being closed — because they all funnel through the same active-id signal. The repaint is deferred to a microtask so the effect never writes the active-id signal re-entrantly.
