...
F5 / View → Refresh fans out across **every working surface** (`reloadAll` in `src/renderer/hooks/use-conception.ts`): it drops the git-status TTL cache, then reloads projects (which also refreshes the derived Deliverables view), knowledge, resources, all four skill tabs, agents, tasks, and logs, and calls the full `reloadRepos()` so any out-of-app worktree mutation is visible immediately. The agents/tasks panes own their own `createResource` and logs pushes a refresh through a deferred signal, so those three are kicked synchronously rather than awaited alongside the store reloads.
