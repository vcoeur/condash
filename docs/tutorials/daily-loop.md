...
The launcher command lives in `configuration.json`, under `open_with.main_ide.command` — a single command string with `{path}` substituted at launch time. See [Repositories and open-with buttons](../guides/repositories-and-open-with.md) for how to wire your own editor in.
