...
An incident card looks like a project card. What marks it as one is the **kind glyph** at the head of the title — every card whose kind parsed carries one, a small monochrome outline (a triangle for an incident, a diamond for a project, a page for a document; hover it for the word). `Environment` and `Severity` are frontmatter fields you set when creating the item; they live in the README, not on the card.
...
Clicking the entry opens the PDF in an in-app modal hosting a Chromium `<webview>` — no OS handler involved. The **Files** panel next to Steps is a live tree of the item's own directory, with `+ Add note` / `+ New file` / `+ New folder` to grow it without leaving the app. There is no deliverable badge on the card itself; the card carries its dated slug on its first row, the kind glyph and title on the second, and a meta row with app pills, branch, open-PR badges, a warn glyph for a non-canonical status, the step progress, and the last-activity date. See [Deliverables and PDFs](../guides/deliverables.md).
