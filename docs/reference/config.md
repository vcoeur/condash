...
- **`taskConfig`** — per-task scheduling + run mode + log routing, keyed by slug, in **`.condash/settings.json`** (or overridden in `condash.json`) — *not* in `task.json`. Each entry is `{ schedule?, timeout?, runMode?, excludeFromLogs? }`:
    - `schedule` — opt-in cadence (`s` / `m` / `h` / `d`, e.g. `5m` / `1h` / `7d`). The editor offers a fixed list (Off, 1m, 5m, 30m, 1h, 2h, 6h, 12h, 1d, 7d). A scheduled task runs **headless** (no tab) on that interval, single-flighted + per-tab growth-gated (a tick with no changed tab is skipped; the changed subset is handed to the run as `{UPDATED_TABS}`); its console output is teed to `.condash/scheduled/<slug>/` (last ~5), **never** the normal logs. No default schedule; the task must carry a prompt-seedable agent.
