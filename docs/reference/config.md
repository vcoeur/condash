...
| `taskConfig` | both | object | — | Per-task `{schedule?, timeout?, excludeFromLogs?}` keyed by slug — opt-in headless scheduling + run timeout + run-log routing. [↓](#tasks) |
...
- **`taskConfig`** — per-task scheduling + log routing, keyed by slug, in **`.condash/settings.json`** (or overridden in `condash.json`) — *not* in `task.json`. Each entry is `{ schedule?, timeout?, excludeFromLogs? }`:
    - `schedule` — opt-in cadence (`s` / `m` / `h` / `d`, e.g. `5m` / `1h` / `7d`). The editor offers a fixed list (Off, 1m, 5m, 30m, 1h, 2h, 6h, 12h, 1d, 7d). A scheduled task runs **headless** (no tab) on that interval, single-flighted + growth-gated; its console output is teed to `.condash/scheduled/<slug>/` (last ~5), **never** the normal logs. No default schedule; the task must carry a prompt-seedable agent.
    - `timeout` — per-run hard cap (same cadence syntax; absent = `10m`). The run is killed and discarded once it elapses — the discard mechanism for agents that finish their work but don't exit. Keep it ≤ `schedule` or single-flight stretches the effective cadence to the timeout. The editor offers 1m / 5m / 10m / 30m / 1h.
