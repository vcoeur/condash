...
| `autoRefreshOnTabSwitch`    | `false`                                                 | Live full-screen TUIs (alt-buffer tabs) are auto-repainted on switch regardless of this setting, since their hydrated frame is inherently lossy. When `true`, the auto-repaint additionally covers **every** tab — plain shells too — i.e. switching to any tab acts like pressing **Refresh**. See [Hidden terminal tabs parse off the main thread](../explanation/internals.md#terminal-worker). |
