...
| `autoRefreshOnTabSwitch`    | `false`                                                 | When `true`, switching to a terminal tab auto-runs the **Refresh** (repaint) action on the newly-active tab, so a live full-screen TUI never shows a stale hydrated frame after the hidden-tab serialize/hydrate round-trip. Same effect as pressing Refresh on every switch; plain shells are unaffected. See [Hidden terminal tabs parse off the main thread](../explanation/internals.md#terminal-worker). |
