...
| `long_lived_branches` | Branch name patterns that `condash worktrees remove` refuses to delete. Supports glob wildcards `*` (any run of characters) and `?` (one character) — e.g. `main`, `release/*`, `hotfix-?`. Defaults to `["main", "master"]` when unset, so leaving it empty does **not** disable protection; list patterns here only to broaden it (e.g. add `release/*`, `preprod`). Edited under **Settings → Workspace & paths**. |
