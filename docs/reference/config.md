...
| `layout`                | global      | object       | —        | Persisted pane layout, including `leftView` (`projects` \| `tasks` \| `deliverables` \| `perf`). [↓](#layoutstate)                                                                                                                 |
...
| `leftView`      | `'projects' \| 'tasks' \| 'deliverables' \| 'perf'`                  | Which pane fills the left band — the Projects list, the Tasks list, the Deliverables aggregation of every project's `## Deliverables`, or the Performance pane. Selected by the left activity rail, in that order. Defaults to `'projects'`. A persisted `'outputs'` (v3.20.0) is migrated to `'deliverables'`. |
