...
Every top-level key, in one place. **Scope** is which file the key is valid in: _both_ keys act as a per-machine default in `settings.json` and a per-conception override in `.condash/settings.json`; _global-only_ keys are rejected in a conception file (a tree cannot describe its own location).
...
| `taskConfig`            | both        | object       | —        | Per-task `{schedule?, timeout?, runMode?, excludeFromLogs?, gateOnUpdatedTabs?}` keyed by slug — opt-in headless scheduling + run timeout + run mode (`--prompt`/`--run`) + run-log routing + activity gate. [↓](#tasks) |
...
Task _definitions_ are **not** a config key — they live on disk at `<conception>/tasks/<slug>/` (see [Tasks](#tasks)). Their per-task **scheduling / log-routing** lives in the `taskConfig` key above, keyed by slug. condash also persists a few small UI-state fields it manages itself (e.g. `skillsActiveScope`, the last-active Settings tab) in `settings.json`; you don't edit those by hand. Strict-mode validation rejects any unknown top-level key on save.
...
Use absolute paths for `workspace_path` / `worktrees_path` — there is no `~` expansion for path keys. (The one place `~` works is a leading `~/` in an `open_with` _command_ token — see [Per-OS recipes](#per-os-recipes).) JSON does not carry comments — keep prose documentation in the project README or the per-tree `CLAUDE.md`.
...
The `dashboard` block configures **live terminal-tab summarization**: a periodic loop in the main process reads the recent output of the open terminal tabs and summarizes it with the [pi coding-agent SDK](https://pi.dev) on a DeepSeek key, surfacing the result as (1) LLM-derived tab titles, (2) a hover popover, and (3) the **Dashboard** body in the bottom band — a handle next to **Terminal** (`Ctrl+Shift+D`) swaps the band between the terminals and the dashboard. It is **off by default** — nothing runs and no data leaves the machine until you enable it. State (per-tab summaries + a rolling event history) persists at `<conception>/.condash/dashboard/state.json`.

Edit it in **Settings → Dashboard**. The secret `apiKey` and the endpoint (`baseUrl` / `model`) live on the **Global** tab (per-machine `settings.json`) and must never be committed to a conception's `condash.json`. The **This conception** tab can override only `enabled` and `intervalSec` for the current tree — the effective config merges the `dashboard` block one level deep, so those two overlay the global key/URL/model rather than replacing them. The Global tab also has a **Test connection** button that runs a one-shot completion against the entered key/URL/model.
...
- **`taskConfig`** — per-task scheduling + run mode + log routing, keyed by slug, in **`.condash/settings.json`** (or overridden in `condash.json`) — _not_ in `task.json`. Each entry is `{ schedule?, timeout?, runMode?, excludeFromLogs?, gateOnUpdatedTabs? }`:
