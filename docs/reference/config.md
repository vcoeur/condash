...
| `{"name": "repo", "label": "<text>"}` | Optional human-friendly label rendered as a small subtitle / pill next to the repo name on the Code tab. Useful when the directory name is a slug (`alicepeintures.com`) and a friendlier descriptor (`Alice's site`) gives quicker context. Works on both top-level entries and submodules; combinable with `run` / `force_stop` / `submodules`. |
  ...
  "terminal": {
    "xterm": {
      ...
      "colors": {
        ...
        "white": "#bac2de",   "bright_white":   "#a6adc8"
      }
    }
  }
}
