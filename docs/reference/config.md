...
| `repositories`          | conception  | array        | `[]`     | Ordered Code-pane repo list; also the app registry (`#handle`, `label`, `purpose`, `aliases`, `run`, `submodules`, `section`). [↓](#repositories)                                                                                   |
...
| `starredProjects`       | conception  | array        | `[]`     | Slugs starred on the Projects pane — a starred card shows a filled star and sorts to the top of its status section. Written by the card star, not by hand; a done item's slug is dropped. [↓](#starredprojects)                |
...
| --------------- | ------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enabled`       | boolean       | `false` | Toggle capture entirely. Default flipped to opt-in for privacy (2.25.0). Disabling stops new writes on the next session spawn; existing files stay on disk for the janitor. The Logs surface stays usable for browsing past transcripts even when disabled. |
...
| `skipIdle`       | boolean     | `true`           | The narrower backstop under `gateOnActivity`: even with the gate **off**, hold back a due tab whose last card already read `idle` and whose byte count has not moved. Turn it off only when you want an unconditional refresh of every tab every cycle. |
...
- The **Skills** pane reads the [agedum](skill.md#the-harness-launcher-agedum) sources at `<conception_path>/.agents/skills/` (and `~/.config/agents/skills/` for the User scope). The former `skills_path` key was dropped; condash never reads compiled per-harness outputs (`.claude/`, `.kimi/`, …). See the [Skills pane guide](../guides/skills-pane.md).
...
| `{"name": "repo", "run": "<cmd>", "force_stop": "<cmd>"}` | Same as above plus a repo-level **force-stop** button, for freeing a port held by a server condash didn't start. Unlike `run`, `force_stop` is **not** run through a shell: the string is split into argv and spawned directly, so pipes, `&&`, globs and `${VAR}` do **not** work — use a single command that does the job by itself. **Per-OS recipes for "kill whatever is holding port 8300":** Linux `fuser -k 8300/tcp` or `pkill -f 'manage.py runserver'`; macOS `pkill -f 'node .*8300'`; Windows `taskkill /F /IM node.exe`. Same trust level as `run` — you're running these commands on your own machine. |
| `{"name": "repo", "label": "<text>"}`                     | Optional human-friendly label. The Code-pane card's primary pill is always the `#handle`; the label renders as a secondary subtitle beside it (falling back to the directory name when it differs from the handle). Useful when the handle is terse and a friendlier descriptor (`Helio Web`, `Helio Docs`) gives quicker context. Works on both top-level entries and submodules; combinable with `run` / `force_stop` / `submodules`.                                                                    |
| `{"name": "repo", "purpose": "<text>"}`                   | One line on what the app is for. `condash applications sync-docs` renders it as the **Purpose** column of the `AGENTS.md` Apps table, so a reader gets the answer without opening a second file and no hand-kept companion table is needed. Set it with `condash applications set <handle> --purpose "…"`; an empty value clears it. Pipes and newlines are escaped and flattened when the table is rendered. |
...
| `{"name": "repo", "env": [".env", ".env.local"]}`         | Files copied from the primary checkout into the worktree on `condash worktrees setup` — applied **unconditionally** when present (no flag needed); `--no-env` skips it. A new worktree gets every declared file (sources absent from the primary checkout are skipped); re-running setup on an already-present worktree backfills **only the declared files it is missing**, so a deliberately divergent copy (different ports to run two branches side by side) survives. Closes the silent-undefined-`VITE_*` footgun where a worktree without env wiring leaves a Vite SPA reading `import.meta.env.VITE_*` as `undefined`. Default empty → no copy. Path traversal is rejected (no `..`, no absolute paths).                                                                                                                         |
...
**A done item's slug is dropped from the list.** A star pins what is live, so it does not survive the close. The Projects pane reconciles the list against the projects it has loaded, rather than the close transition writing the removal, which is what makes every route to `done` equivalent: the pane's own status change, a `condash projects close` that reaches the open app as a file-watcher event, a hand-edited README, and the stars already stranded on items closed before the rule existed. The shrunk list is written back, so a reopened item comes back unstarred.
...
| `logging`                   | `{}` (disabled)                                         | Per-session terminal capture under `.condash/logs/` — see [Terminal logging](#terminal-logging) above. Off by default.                                                               |
...
| `autoRefreshOnTabSwitch`    | `true`                                                  | When `true` (default), any tab coming into view automatically runs **Refresh** — full-screen TUIs, plain shells, and agent sessions are all repainted, so a hidden tab never shows a stale snapshot. That covers a tab switch, the first tab shown after a restore or spawn (once per column, not once per restored tab), the tab that takes over when the active one closes (or exits cleanly), and the Dashboard→terminal flip / pane reopen (both columns of a split). Set explicitly to `false` to restrict auto-refresh to alternate-buffer tabs only (the previous default). See [Hidden terminal tabs parse off the main thread](../explanation/internals.md#terminal-worker). |
...
- **`taskConfig`** — per-task scheduling + run mode + log routing, keyed by slug, in **`.condash/settings.json`** — _not_ in `task.json`. It is a conception key (it describes this tree's tasks) and lives only in the conception file. Each entry is `{ schedule?, timeout?, runMode?, excludeFromLogs?, gateOnUpdatedTabs? }`:
  ...
  Both segregated stores are browsable from the Logs surface's **Task runs** view and stay invisible to the normal Logs list, search, and reports.
...
Visual + behavioural knobs for the xterm.js renderer. All keys are optional; missing keys fall through to xterm's defaults. Edit through the **Settings → Terminal** section — `terminal` is a personal/per-machine key, so the editor writes the global `settings.json` (**not** `.condash/settings.json`) and reloads existing tabs without a relaunch.
...
| `scrollback`       | non-negative int                    | Lines retained per tab. Default 5 000 — the same figure the session logger sizes its headless buffer against.                                                            |
...
| `theme`                 | A theme preset id (`light`, `mist`, `dark`, `nocturne`, `console`) or `system`. Persisted by `setTheme`. See [Theme](#theme).                                                                                                                                                                                                      |
...
| `terminal.*`            | Embedded-terminal preferences. See [`terminal`](#terminal) above for every sub-key.                                                                                                                                                                                                                            |
| `layout`                | Composite-layout state. See [LayoutState](#layoutstate) below.                                                                                                                                                                                                                                                 |
| `welcome`               | First-launch state. `welcome.dismissed: true` hides the Welcome screen even when both Projects and Knowledge are empty. `welcome.initShown: false` is the one-shot post-template-init marker — the init sets it so the screen appears once even though the seeded tree has knowledge entries; the first render persists `true`. |
...
| `logs`         | int 120 – 2400 (px) | 400     | Min width of a session card on the Logs surface.             |
| `tasks`        | int 120 – 2400 (px) | 340     | Min width of a task card on the Automations surface.         |
| `deliverables` | int 120 – 2400 (px) | 340     | Min width of a deliverable card in the Deliverables section. |
...
| `mist`     | Mist         | light | Cool morning light — fog gray-blue, ink text, steel-blue accent.   |
| `dark`     | Warm Gallery | dark  | Gold on warm black — the gallery-dark lead theme.                  |
| `nocturne` | Nocturne     | dark  | Moonlit indigo night — periwinkle on deep blue-black.              |
...
the token set cannot reach, in `renderer/theme-console.css`. The active terminal
tab is **reverse-video** (filled with its own accent, ink printed on top)
instead of tinted — the active rail item got that same treatment as the base
rule in every theme (2026-08, rail-visibility); pane titles become a tracked-out
uppercase label behind an accent `›` with a hairline running out to the actions,
in place of the editorial serif title and its slide-rule; status dots are
**squares** and the chips lose their capsule ends; and step-progress bars are
**segmented** rather than solid. App pills stay filled — that fill is the
per-app hash-hue channel, not decoration. No other preset is affected: every
rule in that sheet is scoped to `[data-theme='console']`.
