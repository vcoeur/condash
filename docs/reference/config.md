...
| `writerModel`    | string      | `deepseek-v4-pro` | **Writer model** — the richer tier that composes each card's one-sentence **subtitle** (the work's context and purpose) from the card facts plus the tab's derived provenance (app / worktree / project). Defaults to `model` when a single-tier config is in use (so a single-model endpoint keeps working). |
...
| `writerReasoning`| boolean     | `true`           | Whether the writer model reasons. On by default: composing the per-tab subtitle is the one place reasoning measurably improves the wording.                                              |
