...
| `autoRefreshOnTabSwitch`    | `true`                                                  | When `true` (default), any tab coming into view automatically runs **Refresh** — full-screen TUIs, plain shells, and agent sessions are all repainted, so a hidden tab never shows a stale snapshot. That covers a tab switch, the first tab shown after a restore or spawn, the tab that takes over when the active one closes (or exits cleanly), and the Dashboard→terminal flip. Set explicitly to `false` to restrict auto-refresh to alternate-buffer tabs only (the previous default). See [Hidden terminal tabs parse off the main thread](../explanation/internals.md#terminal-worker). |
