...
| `dashboard`             | global      | object       | —        | Live terminal-tab summarization (pi SDK + DeepSeek): `{enabled, provider, apiKey, model, intervalSec, gateOnActivity, historyLimit}`. Off by default; set it in **Settings → Dashboard**, which writes to the global file (the `apiKey` is a secret). [↓](#dashboard)                                                                 |
