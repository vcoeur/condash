...
On the Projects pane the edge renders both ways. The child card grows a **Part of ↑** banner carrying the parent's title and status pill; the parent card grows a **Subprojects** fold — a header with the child count, **collapsed by default**, that opens to one clickable row per child with its own status pill, ordered by status then slug (the open/closed state is remembered per parent, in the same browser-local store as the section collapse). Both banner and rows are buttons that open the referenced item. Both sides of the relation share one **family colour** on the card frame — the only colour a card frame ever carries; standalone cards stay neutral — plus a distinct edge (thick left spine on the parent, dashed sides on the child). A dangling `parent:` degrades to the raw slug as non-clickable text rather than disappearing, and that card takes no family colour — there is no second card for a hue to tie it to.
