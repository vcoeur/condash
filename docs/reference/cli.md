...
| `links` | Relative Markdown links under `knowledge/` and `projects/` whose target or heading anchor no longer resolves. URL-encoded paths, duplicate GitHub-style heading slugs, and explicit HTML `id` / `name` anchors are supported; fenced and inline-code examples plus external schemes are ignored. Also flags broken `**Transferred:**` targets in bare-backticked and Markdown-link forms. Never auto-fixed because a valid repair needs editorial judgment |
