...
6. Groups eligible paths: one commit per item (`<YYYY-MM-DD-slug>: sync`), one for knowledge (`knowledge: sync`), one for every non-tree change (`meta: sync`), one for the regenerated indexes (`indexes: sync`) — the index commit always lands after the item commits it refers to.
