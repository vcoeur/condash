...
3. Considers paths under `projects/` and `knowledge/`, plus the root-level structural files sync owns — `AGENTS.md`, `README.md`, `.gitignore`, `.gitattributes`, and everything under `.agents/`. That allowlist is deliberate: a stray root file is no more sync's to commit than `projects/stray.md` is, and the per-harness *generated* views (`.claude/`, `.kimi/`, `.pi/`, `opencode.json`, `CLAUDE.md`) are left alone — they should be gitignored, not swept. Gitignored paths (`resources/local/`, `projects/**/local/`, the `.index-dirty` sentinels) never enter a commit.
...
6. Groups eligible paths: one commit per item (`<YYYY-MM-DD-slug>: sync`), one for knowledge (`knowledge: sync`), one for the root structural files (`meta: sync`), one for the regenerated indexes (`indexes: sync`) — the index commit always lands after the item commits it refers to.
...
Paths under `projects/` or `knowledge/` that match no known shape (say `projects/stray.md`) are reported under `skipped[]` with reason `unresolved` and are never committed. A path outside the trees that is not on the structural allowlist is ignored entirely — not committed, not reported.
