...
| `close <slug>` | Set status to `done` (or `--status <name>`) and append a `Closed.` timeline entry, annotated with `--summary <text>` when given. Ordinary close does **not** record a knowledge-promotion acknowledgement. Pass `--knowledge-checked` only after the editorial review and only with a `done` target; it appends dated `Checked knowledge promotion` after `Closed.` as the last timeline entry. `--no-touch-dirty` skips touching `projects/.index-dirty`, for a caller that regenerates the indexes itself |
...
| `check-knowledge <slug> [--record]` | Signal whether a `done` project still needs a knowledge-promotion check (read-only). `--record` remains the standalone recorder: after a real review it appends dated `Checked knowledge promotion` as the last timeline entry. `close --knowledge-checked` is the equivalent recorder when closing after that review. No mass/backfill writer: the marker is only ever written for a project that was actually reviewed |
...
condash audit --include links        # check relative Markdown links and anchors
```
...
| `links` | Relative Markdown links under `knowledge/` and `projects/` whose target or heading anchor no longer resolves. CommonMark AST positions provide exact link and reference-link lines; GitHub-compatible slugs cover decoded fragments, duplicate headings, combining marks, and image alt text. Explicit HTML `id` / `name` anchors are supported; fenced and inline-code examples plus external schemes are ignored. Also flags broken `**Transferred:**` targets in bare-backticked and Markdown-link forms. Never auto-fixed because a valid repair needs editorial judgment |
...
| `check-knowledge` | `done` projects whose last timeline entry isn't `Checked knowledge promotion` — the explicit promotion-review acknowledgement is missing or stale. Resolve by doing the real `/knowledge` review, then `projects check-knowledge <slug> --record`; when the review is completed before closing, `projects close <slug> --knowledge-checked` records it last. Legacy done projects stay flagged until actually reviewed (no backfill shortcut) |
...
further: a missing ```svg fence or a first element that is not `<svg>` is an
error (an XML prolog, DOCTYPE or comment before the root is accepted, as a
tool export carries), and it warns on a root without `viewBox`, a block
without `alt`, and any element the viewer's sanitizer will strip — `<style>`,
`<script>`, `<foreignObject>`, `<use>`, `<animate>`, `<animateMotion>`,
`<animateTransform>`, `<set>` (one list, `SVG_STRIPPED_ELEMENTS`, shared with
the sanitizer). Nested svg blocks inside `tabs` get the same checks.
