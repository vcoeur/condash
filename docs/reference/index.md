...
- [CLI](cli.md) — every noun and verb on the command line.
- [Config files](config.md) — `.condash/settings.json` (per-tree) and `settings.json` (per-machine).
...
- [Mutation model](mutations.md) — every line condash writes.
...
- [Management skills](skill.md) — the five shipped skills: `/projects`, `/knowledge`, `/pr`, `/applications`, `/visual`.
