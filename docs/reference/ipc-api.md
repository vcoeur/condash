...
| `getStarredProjects()` | `string[]` | Slugs starred on the Projects pane, from the conception-scoped `starredProjects` config key ([config](config.md#starredprojects)). Deduped and sorted; empty when nothing is starred or no conception is set. |
...
| `searchProjectReadmes(query)` | `string[]` | The Projects-pane [filter bar](../guides/projects-pane.md#filter)'s search: the `Project.path` of every item whose `README.md` content or slug carries every term of `query` — same grammar and index as `search`, but a boolean predicate (no scoring, no snippets, no match on the rest of the path), README files only, and **uncapped** (a filter must be exhaustive where the ranked search only has to be useful). Served from the in-memory index; the scan yields to the event loop every 64 files so a large corpus can't monopolise the main thread. |
...
| `setProjectStar(slug, starred)` | Star or unstar one project, writing the conception's `starredProjects` list — a **config** write, not a tree write, so no README is touched and nothing lands in git. Returns the new full slug list; the key is removed when the list empties. |
| `pruneStarredProjects(doneSlugs)` | Unstar every slug in `doneSlugs`, in **one** config write, and return the surviving list. A star pins a live item to the top of its section, so it does not outlive the close. The caller passes the done slugs because the Projects pane already holds the parsed project list — deriving them in main would walk and parse the whole tree a second time. A starred slug absent from the argument is left alone, so a partial list under-prunes rather than over-prunes; the key is removed when the list empties. |
...
| `exportNotePdf(path, html)` | Export a rendered note as a PDF. `html` is the self-contained document the renderer built (`note-modal-parts/export-pdf.ts` for markdown notes, `mdx-modal-parts/export-pdf.ts` for visual notes); `path` is the source note, used only to seed the save dialog's default `<name>.pdf`. Main pops the save dialog, prints the document via `printToPDF` in a hidden window, and writes the result. Returns the saved path, or `null` on cancel. |
| `saveSvg(defaultPath, svg)` | Save an `svg` block's diagram as a standalone `.svg`. `svg` is the sanitized, token-resolved markup the viewer drew; `defaultPath` only seeds the save dialog (`<note-stem>-<block-id>.svg` beside the note) and is never read or written itself. Main pops the dialog and writes the text to the picked path. Returns the saved path, or `null` on cancel. |
...
| `listReposForPrimary(name)` | Per-primary partial reload — returns the primary's `RepoEntry` plus its submodule children freshly re-read. Driven by the structural FS-watcher event `repo-worktrees-changed`. The watch set is re-synced **incrementally** for the affected primary (its old contribution replaced by the fresh entries' targets — add/remove only the delta), not by re-running the whole-registry `listRepos`. |
...
| `listOpenPullRequests(app)` | List every open GitHub PR (with its `headRefName`) for the repo the `apps:` token `app` resolves to — `gh pr list --state open`, one call per repo. Backs the **Projects-pane card badges**: the renderer indexes the results by head branch so each project card matches its own branch without a per-card call, and only polls **while the Projects pane is visible** (badges render nowhere else). `app` is resolved to the configured repo via the name / `#handle` / alias map (never a renderer-supplied path). Returns `[]` for an unknown app or a lookup that can't run. TTL-cached by repo path; the 15 s timeout is enforced from a worker thread (`spawnDeadlineMs`) so a blocked main event loop can't stretch it. The batch is shaped on both sides: concurrent calls for the same repo share one in-flight lookup (single-flight — every `apps:` spelling of a repo collapses to one `gh`), the renderer coalesces re-triggers behind a ~1 s trailing debounce, and each batch drains through a 6-wide pool that merges badges into the index per entry as each lookup lands. |
