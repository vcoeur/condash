...
| `searchProjectReadmes(query)` | `string[]` | The Projects-pane [filter bar](../guides/projects-pane.md#filter)'s search: the `Project.path` of every item whose `README.md` matches `query` — same grammar and index as `search`, README files only, and **uncapped** (a filter must be exhaustive where the ranked search only has to be useful). |
