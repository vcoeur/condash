...
| `getStarredProjects()` | `string[]` | Slugs starred on the Projects pane, from the conception-scoped `starredProjects` config key ([config](config.md#starredprojects)). Deduped and sorted; empty when nothing is starred or no conception is set. **Prunes done items** — a star pins what is live, so any slug whose item is at `status: done` is dropped here *and written out of the config*, which is what makes every route to `done` (the pane, `condash projects close`, a hand-edited README) end up unstarred. A slug that resolves to no item at all is left alone. |
