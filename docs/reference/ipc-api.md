...
| `listReposForPrimary(path)` | Per-primary partial reload — returns the primary's `RepoEntry` plus its submodule children freshly re-read. The resolved primary path, not a basename, identifies the family so two configured `docs` repositories cannot cross-reload. Driven by the structural FS-watcher event `repo-worktrees-changed`. The watch set is re-synced **incrementally** for the affected primary (its old contribution replaced by the fresh entries' targets — add/remove only the delta), not by re-running the whole-registry `listRepos`. |
